...
func TestAgilityIsPerSatellite(t *testing.T) {
	...
	fast, err := satellites.Profile(ctx, nimble)
	if err != nil {
		t.Fatalf("reading %s: %v", nimble, err)
	}
	slow, err := satellites.Profile(ctx, sluggish)
	if err != nil {
		t.Fatalf("reading %s: %v", sluggish, err)
	}

	if fast.Agility.SlewRateDegS != 4.0 || slow.Agility.SlewRateDegS != 0.5 {
		t.Errorf("rates came back as %v and %v", fast.Agility.SlewRateDegS, slow.Agility.SlewRateDegS)
	}
	...
	if want := 6 * time.Second; fast.Agility.SlewTime(from, to) != want { // 20/4 + 1
		t.Errorf("nimble transition = %s, want %s", fast.Agility.SlewTime(from, to), want)
	}
	if want := 60 * time.Second; slow.Agility.SlewTime(from, to) != want { // 20/0.5 + 20
		t.Errorf("sluggish transition = %s, want %s", slow.Agility.SlewTime(from, to), want)
	}

	// And the roll authority differs, which is what decides whether a candidate
	// is flyable at all.
	steep := domain.Attitude{RollDeg: 40}
	if !fast.Agility.WithinRollAuthority(steep) {
		t.Error("the 50-degree spacecraft refused a 40-degree roll")
	}
	if slow.Agility.WithinRollAuthority(steep) {
...
func TestUnknownSatelliteIsNotFound(t *testing.T) {
	p := pool(t)
	_, err := postgres.NewSatellites(p).Profile(context.Background(), "SAT-NEVER-SEEN")
...
func TestDefaultsAreUsable(t *testing.T) {
	...
	got, err := postgres.NewSatellites(p).Profile(context.Background(), satellite)
	if err != nil {
	...
	if got.Agility.SlewTime(at, at) != got.Agility.SettlingFloor() {
