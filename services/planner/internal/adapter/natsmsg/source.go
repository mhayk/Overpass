...
type Source struct {
	...
	// the reason; everything else on a dead letter is transport detail this
...
func Bind(js nats.JetStreamContext, batch int, wait time.Duration) (*Source, error) {
	s := &Source{
		...
		consumerFor: make(map[string]string, len(app.Streams)),
		inflight:    make(map[string]*nats.Msg),
	}
	for _, binding := range app.Streams {
		s.consumerFor[binding.Stream] = binding.Consumer
	...
	for i := range s.order {
		...
		for _, m := range msgs {
			...
			if convErr != nil {
				...
				// dead-lettering publishes and the critical section here
				// guards a map, not a network round trip.
...
// is what tells an operator why the count is missing rather than wrong.
//
...
// accept a publish, and buying it back would mean holding poison in memory.
