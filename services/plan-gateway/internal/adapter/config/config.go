...
type Config struct {
	...
	// read this API, which is the right default for a service deployed behind
	// a same-origin proxy and the wrong one for the compose stack — where the
	// UI is on :3000 and this is on :8083. See docker-compose.yml.
